...
def main():
    ...
    for n, jobdir in enumerate(jobdirs):
        ...
        doit = False
        if field == 'energy':
            fo = f"{out}/Energy_{jobname}{cutlabel}.txt"
            if not os.path.exists(fo):
                doit = True
                fi = open(fo, 'w')
                fi.write(f"Time, Magnetic_Energy, Thermal_Energy, Gravitational_Potential, Turbulent_Energy\n")
                fi.write(f"Myrs, ergs, ergs, ergs, ergs\n")
        elif field == 'density':
        ...
        #            print(E_pot_list)
                    fi.write(f"{time_myrs.value}, {E_B_sum.value}, {E_therm_sum.value}, {E_pot_sum.value}, {E_turb_sum.value} \n")
        ...
        ax.legend()
        ax.set_ylim([0, None])
        plt.savefig(f"{out}/fig_energy_{jobname}{cutlabel}.png", dpi=300)
        ...
        ax.legend()
        ax.set_ylim([0, None])
        plt.savefig(f"{out}/fig_virial_{jobname}{cutlabel}.png", dpi=300)
